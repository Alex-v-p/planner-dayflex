...
def test_ai_client_maps_successful_schedule_explanation(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(
        httpx,
        ...
        lambda *args, **kwargs: httpx.Response(
            ...
            json={
                ...
                "confidence": 0.7,
                "explanation": (
                    "Write report moved because unavailable time changed the day."
                ),
                "fallback_reason": None,
                "error_code": None,
            },
    ...
    result = HttpAiClient("http://ai.test").explain_schedule_decision(
        {
...
def test_ai_client_explanation_falls_back_for_network_errors(
    monkeypatch: pytest.MonkeyPatch,
    exception: httpx.HTTPError,
    reason: str,
    code: str,
    ...
    result = HttpAiClient("http://ai.test").explain_schedule_decision(
        {
            "reason_code": "moved_after_interruption",
            "deterministic_reason": "Write report was moved.",
            "facts": {"task_title": "Write report"},
        }
    )
...
@pytest.mark.parametrize(
    ...
    [
        (
            httpx.Response(500, json={"detail": "down"}),
            "service_unavailable",
            "ai_service_non_success",
        ),
        (
        ...
        (
            httpx.Response(200, json={"status": "explained"}),
            "invalid_response",
            "ai_service_invalid_response",
        ),
        (
...
def test_ai_client_explanation_falls_back_for_unusable_responses(
    monkeypatch: pytest.MonkeyPatch,
    response: httpx.Response,
    reason: str,
    code: str,
